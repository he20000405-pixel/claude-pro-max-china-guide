...
保存报错原文、发生时间、购买入口、目标套餐和银行交易状态。若已经收到 Anthropic 收据，或银行显示已完成扣款或“待处理”，请停止再次提交支付，先核对原订单。
...
| 银行显示“待处理” | 银行已经收到付款请求，但尚未确认最终扣款 | 等银行或账单页面给出最终结果，不立即再买 |
...
重新提交后若页面进入 3DS 或订单确认，应完成当前流程并等待结果。若仍出现相同拒付，停止重试，带着报错和时间联系发卡行或 Anthropic 支持。
